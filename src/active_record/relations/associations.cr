module CQL
  module ActiveRecord
    ...
    class AssociationMetadata(T) < BaseAssociationMetadata
      getter target_class : T.class = T
    ...
    module AssociationStorage
      ...
      @loaded_associations : Hash(Symbol, Array(T)) = {} of Symbol => Array(T)

      # Sets a loaded association
      # - **param** : name (Symbol) - The name of the association
      # - **param** : records (Array(CQL::Model)) - The loaded records
      # - **return** : Nil
      def set_loaded_association(name : Symbol, records : Array(T))
        @loaded_associations[name] = records
      end

      # Gets a loaded association
      # - **param** : name (Symbol) - The name of the association
      # - **return** : Array(CQL::Model)?
      def get_loaded_association(name : Symbol) : Array(T)?

...
CQL provides various methods to retrieve records from the database. Many of these are covered in detail in the [Querying Guide](./querying.md).
...
For building more complex queries (e.g., with joins, specific selections, grouping), refer to the [Querying Guide](./querying.md).
...
This guide covers the primary CRUD operations available in CQL Active Record. For more advanced querying, refer to the [Querying Guide](./querying.md), and for information on lifecycle events and data integrity, see the guides on [Callbacks](./callbacks.md) and [Validations](./validations.md).

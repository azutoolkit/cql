...
- `->(optional_arguments) { ... }`: A proc that defines the query logic.
  ...
  - Inside the proc, you use standard CQL query methods like `where`, `order`, `limit`, etc. These methods are called on the current query context, which is typically a `Query(YourModel)` instance or the model class itself if starting a new chain.
  - The proc should return a `CQL::Query` or a `Query(YourModel)`. If a raw `CQL::Query` is returned (e.g., by starting with `query.where(...)`), the `scope` macro intelligently wraps it in a `Query(YourModel)` to ensure it remains chainable with other scopes or Active Record query methods.
...
- `Article.published` returns a `Query(Article)`.
- `Post.created_after(7.days.ago)` returns a `Query(Post)`.
...
#     Query(Article).new(scope_call_result)
#   else
#     scope_call_result # Assumed to be Query(Article)
#   end
# end
```
...
When defining scopes as class methods, they should generally return a `Query(YourModel)` instance to maintain chainability. The `Queryable` module (included via `CQL::ActiveRecord::Model`) provides methods like `where`, `order`, etc., that return `Query(YourModel)` instances and can be used here.
...
Scopes returning `Query(YourModel)` are designed for chaining with each other and standard query methods (`.where`, `.order`, etc.):
...
The `Query` class often uses `forward_missing_to Target` (where `Target` is your model class). This allows class methods on your model (including those generated by the `scope` macro or defined manually) to be called on a `Query` instance, enabling natural chaining like `Article.where(...).published`.
